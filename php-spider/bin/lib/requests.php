...
class requests
{
	...
	public static function get_response_bodys($html,$domain)
	{
		$header = $body = '';
		$http_headers = array();
		// 解析HTTP数据流
    ...
    /**
     * 多进程curl
     * @author  li shao wei  <980167048@qq.com>
     * @access  public
     * @param $urls
     * @param array $fields
     * @param int $wait_usec
     ...
     * @return  array|bool
     */
    public static function gets($urls,$fields=array(),$wait_usec=0)
    {
        if (!is_array($urls))
        return false;
        ...
        foreach($urls as $key=>$url){
            self::inits();
            self::http_clients($url,'get',$fields);

            curl_multi_add_handle($mh, self::$ch); // 把 curl resource 放进 multi curl handler 里
            $handle[$key] = self::$ch;//
            $data[$key]['url']   = $url;
        }
        /* 执行 */
        ...
        foreach($handle as $i => $ch) {
            ...
            $data[$i]['code']  = curl_getinfo(self::$ch,CURLINFO_HTTP_CODE);
            $data[$i]['html'] = (curl_errno($ch) == 0) ? self::get_response_bodys($content,self::$domains[$i]) : false;
        }
        /* 移除 handle*/
    ...
    public static function http_clients($url,$method = 'GET',$fields)
    {
    ...
    public static function http_client($url, $method = 'GET', $fields)
    {
        ...
        if ($method == 'GET' && !empty($fields)) 
        {
            $url = urldecode($url . (strpos($url,"?")===false ? "?" : "&") . http_build_query($fields,','));
        }
        $parse_url = parse_url($url);
        if (empty($parse_url) || empty($parse_url['host']) || !in_array($parse_url['scheme'], array('http', 'https'))) 
        {
